...
;=======================================================================================================================================================================================================================
TargetFunc:
IniRead, Safe_Mode, %EndConfigSuporte%.ini, Principal, Safe_Mode
IniRead, Safe_StopAttack, %EndConfigSuporte%.ini, Principal, Safe_StopAttack

			ImageSearch, FoundX, FoundY, %Tibia_X%, %Tibia_Y%, %Tibia_Width%, %Tibia_Height%, *50, Imagens\%VersaoGrafico%\checaclienteaberto.png	; Checa coracao do cliente
			if (ErrorLevel = 1) {
				goto TargetFunc
			}
			
if (SAFE_MODE = 1) {
	ImageSearch, XBattleAlert, YBattleAlert, %Tibia_X%, %Tibia_Y%, %Tibia_Width%, %Tibia_Height%, *15, Imagens\%VersaoGrafico%\BattlePlayerAlert.png	; 
		if (ErrorLevel = 1) {
			SoundBeep, 750, 500
			if (Safe_StopAttack = 1) {
				ControlSend,, {Escape}, ahk_pid %Actual_PID% 
				ControlSend,, {Escape}, ahk_pid %Actual_PID% 
				ControlSend,, {Escape}, ahk_pid %Actual_PID% 
			}
			goto TargetFunc
			...
			ImageSearch, XBattle, YBattle, %Tibia_X%, %Tibia_Y%, %Tibia_Width%, %Tibia_Height%, *15, Imagens\%VersaoGrafico%\BattleList.png	; 
				if (ErrorLevel = 1) {
				goto OutBattle
				}
				if (ErrorLevel = 0) {
				...
				XFimBuscaTarget = % XBattle + 66
				YFimBuscaTarget = % YBattle + 140
			...
			OutBattle:
				...
				if ((Ativa_Atk = 1) and (ForaPZ = 1)) {
						PixelSearch, Px, Py, %XBattle%, %YBattle%, %XFimBuscaTarget%, %YFimBuscaTarget%, 0x00C000, 0, FAST RGB
							if (ErrorLevel = 0){
								TemMobBattle = 1
								goto achoutarget
							} else {
						...
						PixelSearch, Px, Py, %XBattle%, %YBattle%, %XFimBuscaTarget%, %YFimBuscaTarget%, 0x60C060, 0, FAST RGB 
							if (ErrorLevel = 0){
								TemMobBattle = 1
								goto achoutarget
							} else {
						...
						PixelSearch, Px, Py, %XBattle%, %YBattle%, %XFimBuscaTarget%, %YFimBuscaTarget%, 0xC0C000, 0, FAST RGB
							if (ErrorLevel = 0){
								TemMobBattle = 1
								goto achoutarget
							} else {
						...
						PixelSearch, Px, Py, %XBattle%, %YBattle%, %XFimBuscaTarget%, %YFimBuscaTarget%, 0xC03030, 0, FAST RGB
							if (ErrorLevel = 0){
								TemMobBattle = 1
								goto achoutarget
							} else {
						...
						PixelSearch, Px, Py, %XBattle%, %YBattle%, %XFimBuscaTarget%, %YFimBuscaTarget%, 0xC00000, 0, FAST RGB
							if (ErrorLevel = 0){
								TemMobBattle = 1
								goto achoutarget
							} else {
						...
						PixelSearch, Px, Py, %XBattle%, %YBattle%, %XFimBuscaTarget%, %YFimBuscaTarget%, 0x600000, 0, FAST RGB 
							if (ErrorLevel = 0){
								TemMobBattle = 1
								goto achoutarget
							} else {
						...
						PixelGetColor, CorBattle, %XNextTarget%, %YNextTarget%, RGB ; 
							if (CorBattle != "0xFF0000") { 
								...
								ControlSend,, {Escape}, ahk_pid %Actual_PID% 
								sleep 100
								ControlSend,, {%Hotkey_Target%}, ahk_pid %Actual_PID% 
								}
							} else {
								TemTarget = 1
							}
	...
	if ((Ativa_Atk = 1) and (TipoTarget = "Next Target") and (TemMobBattle = 1)) {
...
if ((Ativa_Atk == 1) and (ForaPZ == 1) and ((TemTarget = 1) or (TipoTarget = "Desligado"))) {
	
;=========================Rune1=============================
		if (AtkRune1 == 1){
			ControlSend,, {%Rune1%}, ahk_pid %Actual_PID% 
			Sleep %TimeDelay1%
		}
;=========================Rune2=============================
		if (AtkRune2 == 1){
			ControlSend,, {%Rune2%}, ahk_pid %Actual_PID% 
			Sleep %TimeDelay2%
		}
;====================Rotação de Magia 1=====================
		if (AtkRotacao1 == 1){
			...
			ControlSend,, {%HK_Rotacao1%}, ahk_pid %Actual_PID% 
...
GuiControlGet, AtkRotacao2
		if (AtkRotacao2 == 1){
			...
			ControlSend,, {%HK_Rotacao2%}, ahk_pid %Actual_PID% 
