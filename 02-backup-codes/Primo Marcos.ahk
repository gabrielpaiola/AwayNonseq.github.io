...
Process, Close , Primo Marcos-UNICO.exe
...
YTooltip = % Tibia_Height / 2
		...
		Settimer EnviaNumGui, 2000
		Settimer Save_Config, 1500
		Settimer Save_Config_Ataque, 1500
		Settimer CheckValidade, 60000
...
;===============================================================================================
	...
	Gui, Add, DropDownList, x340	y78		w85					vTipoTarget, Desligado||Next Target|First Target
...
GuiControlGet, Mana_AutoHealing
	If ((Life_AutoHealing == 1) or ((Mana_AutoHealing == 1))) {
		process, exist, Heal.exe
		if errorlevel
			goto OutHeal
		else
			run, Complementos\Heal.exe

	} else {
		process, exist, Heal.exe
		if errorlevel
			Process, Close , Heal.exe
	}
OutHeal:

GuiControlGet, Energy_Ring
	If (Energy_Ring == 1) {
		process, exist, SwapUtamo.exe
		if errorlevel
			goto OutUtamoEnergy
		else
			run, Complementos\SwapUtamo.exe

	} else {
		process, exist, SwapUtamo.exe
		if errorlevel
			Process, Close , SwapUtamo.exe
	}
OutUtamoEnergy:
...
GuiControlGet, PotBuff
	...
	If ((EAT == 1) or (Paralize == 1) or (Haste == 1) or (Boots == 1) or (UtamoVita == 1) or (Amulet == 1) or (Ring == 1) or (Quiver == 1) or (PotBuff == 1)) {
		process, exist, Support.exe
		if errorlevel
			goto OutSupport
		else
			run, Complementos\Support.exe
	} else {
		process, exist, Support.exe
		if errorlevel
			Process, Close , Support.exe
	}
OutSupport:
...
GuiControlGet, FOODMP
	if ((SSA == 1) or (Ativa_TankMode = 1) or (MIGHT == 1) or (FOODHP == 1) or (FOODMP == 1)) {
		process, exist, Defender.exe
		if errorlevel
			goto OutDefender
		else
			run, Complementos\Defender.exe
	} else {
		process, exist, Defender.exe
		if errorlevel
			Process, Close , Defender.exe
	}
OutDefender:
...
GuiControlGet, SIORP 
	if ((SIOEK = 1) or (SIORP = 1)) {
		process, exist, Sio.exe
		if errorlevel
			goto OutSio
		else
			run, Complementos\Sio.exe
	} else {
		process, exist, Sio.exe
		if errorlevel
			Process, Close , Sio.exe
	}
OutSio:
...
} ; Cliente na tela / nao treinando
	
	
	if (ManaTraining == 1) {
		process, exist, ManaTrain.exe
		if errorlevel
			goto OutManaTrain
		else
			run, Complementos\ManaTrain.exe

	} else {
		process, exist, ManaTrain.exe
		if errorlevel
			Process, Close , ManaTrain.exe
	}
OutManaTrain:
...
Delay_Consecutive2 = % Delay_Consecutive2 - 1
...
If (((Ativa_QuickLoot_Perm == 1) or (Ativa_QuickLoot == 1)) and (GetPosLoot11X != 0) and (ForaPZ = 1) and (State_Pause1 = "Primo Running")) {
	;if (WinActive("ahk_class Qt5QWindowOwnDCIcon")) {
		QuickLooter()
